...
public interface EpisodeRepo extends JpaRepository<EpisodeDO,Long> {
    ...
    @Query(value = """
        SELECT new ai.enpasos.muzero.platform.agent.e_experience.memory2.ShortEpisode(
            e.id,
            e.ok,
            e.okEpoch )
        FROM EpisodeDO e
...
//    void deleteEpisodesNotOk();
    ...
    @Query("SELECT COUNT(e) FROM EpisodeDO e WHERE e.ok = false")
    long countEpisodesWithOkFalse();
