 ...
 */
public class MySigmoidBinaryCrossEntropyLoss extends Loss {
     ...
     */
    public MySigmoidBinaryCrossEntropyLoss() {
        this("MySigmoidBinaryCrossEntropyLoss");
    }
     ...
     */
    public MySigmoidBinaryCrossEntropyLoss(String name) {
        this(name, 1, false);
    }
     ...
     */
    public MySigmoidBinaryCrossEntropyLoss(String name, float weight, boolean fromSigmoid) {
        super(name);
        this.weight = weight;
    ...
    public NDArray evaluate(NDList label, NDList prediction) {
        ...
        NDArray loss = null;
        NDArray ones = lab.getManager().ones(pred.getShape());
        if (!fromSigmoid) {
            ...
            loss = epsLog(pred).mul(lab)
                .add(
                    epsLog(ones.sub(pred)).mul(ones.sub(lab))
                );
            int i = 42;
        }
        loss = loss.mul(-1f);
