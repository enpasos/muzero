...
public class TrainingConfigFactory {
    ...
    public DefaultTrainingConfig setupTrainingConfig(int epoch, boolean background, boolean isWithConsistencyLoss) {
        ...
        loss.addLoss(new MyIndexLoss(new MyBCESquaredLoss(LEGAL_ACTIONS_LOSS_VALUE + 0, 1f/this.config.getActionSpaceSize(), 1), k));
        k++;
        ...
        for (int i = 1; i <= config.getNumUnrollSteps(); i++) {
            ...
            // legal actions
            log.trace("k={}: LegalActions BCELoss", k);
            loss.addLoss(new MyIndexLoss(new MyBCESquaredLoss(LEGAL_ACTIONS_LOSS_VALUE + i, 1f/this.config.getActionSpaceSize() * gradientScale, 1), k));
            k++;
