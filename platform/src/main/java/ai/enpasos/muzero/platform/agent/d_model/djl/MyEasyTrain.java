...
public final class MyEasyTrain {
    ...
    public static void fit(
        Trainer trainer, int numEpoch, Dataset trainingDataset, Dataset validateDataset,  boolean withValueStdPrediction)
        ...
        for (int epoch = 0; epoch < numEpoch; epoch++) {
            ...
            for (Batch batch : trainer.iterateDataset(trainingDataset)) {
                ...
                // training batch
                trainBatch(trainer, batch, withValueStdPrediction);
    ...
    static boolean withValueStdPrediction;
     ...
     */
    public static void trainBatch(Trainer trainer, Batch batch, boolean withValueStdPrediction) {

        MyEasyTrain.withValueStdPrediction =  withValueStdPrediction;
    ...
    private static Pair<NDList, NDList> reorganizePredictionsAndLabels(Pair<NDList, NDList> input) {
        ...
        IntStream.range(0, numRolloutSteps).forEach(i -> {
                    int extra = withValueStdPrediction ? 1 : 0;
