...
public class InitialInference2ListTranslator implements Translator<List<Game>, List<NetworkIO>> {
    public static List<NetworkIO> getNetworkIOS(@NotNull NDList list, TranslatorContext ctx) {
        ...
        NDArray similarityVector = list.get(1);

      //  NDArray p = logits.softmax(1);

        int size = (int) similarityVector.getShape().get(1);


        float[] simArray = similarityVector.toFloatArray();
...
//
        int n = (int) similarityVector.getShape().get(0);
        ...
        List<NetworkIO> networkIOs = IntStream.range(0, n)
            .mapToObj(i ->
            {
              //  float[] ps = new float[actionSpaceSize];
                ...
                float[] sim = new float[size];
                System.arraycopy(simArray, i * size, sim, 0, size);
