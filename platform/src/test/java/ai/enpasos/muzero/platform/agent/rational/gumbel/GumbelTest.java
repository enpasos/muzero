...
class GumbelTest {
    ...
    void drawActionsTest() {
        ...
        assertEquals(2, drawGumbelActions(actions, 2, 1).size());
        assertEquals(3, drawGumbelActions(actions, 3, 1).size());
        assertEquals(1, drawGumbelActions(actions, 1, 1).size());
        assertEquals(0, drawGumbelActions(actions, 0, 1).size());
//        assertThrows(MuZeroException.class, () -> {
//            GumbelFunctions.drawGumbelActionsInitially(actions, 4);
//        });
        System.out.println(drawGumbelActions(actions, 2, 1).toString());
        System.out.println(drawGumbelActions(actions, 2, 1).toString());
        System.out.println(drawGumbelActions(actions, 2, 1).toString());
        System.out.println(drawGumbelActions(actions, 2, 1).toString());
        System.out.println(drawGumbelActions(actions, 2, 1).toString());
