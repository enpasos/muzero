...
public class GameTree {
    ...
    public GameTree(MuZeroConfig config, List<Integer> startPosition) {
        this.config = config;
        init(startPosition);
    }

    private void init(List<Integer> startPosition) {


        ZeroSumGame startGame = (ZeroSumGame) config.newGame(true, true);
        startGame.apply(startPosition);

        terminalGameNodes = new ArrayList<>();
        nonExpandedGameNodes = new ArrayList<>();

        // fully expand the game tree
        rootNode = new DNode(startGame);
    ...
    public List<DNode> badDecisionFinder(@NotNull GameTree gameTree, @NotNull OneOfTwoPlayer player, boolean withMCTS, Inference inference, int epoch, boolean onOptimalPathOnly) {
        ...
        int[] actions = inference.aiDecisionForGames(gameList, withMCTS );
