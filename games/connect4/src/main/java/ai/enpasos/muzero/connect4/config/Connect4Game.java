...
public class Connect4Game extends ZeroSumGame {

    public static final String PASS = "pass: ";


    public Connect4Game(@NotNull MuZeroConfig config, EpisodeDO episodeDO) {
        super(config, episodeDO);

    }

    public Connect4Game(@NotNull MuZeroConfig config) {
        super(config);
    }

    @Override
    public @NotNull Connect4Environment getEnvironment() {
        return (Connect4Environment) environment;
    }
    ...
    public void replayToPositionInEnvironment(int stateIndex) {
        environment = new Connect4Environment(config);
    ...
    public String render() {
        ...
        if (!this.getEpisodeDO().getActions().isEmpty()) {
            ...
            int colLastMove = ((Connect4Action) action).getCol();

            player = OneOfTwoPlayer.otherPlayer(this.getEnvironment().getPlayerToMove());
            r += player.getSymbol() + " move (" + (((Connect4Action) action).getRow() + 1) + ", " + (char) (colLastMove + 65) + ") index " + action.getIndex();
        }
        r += "\n";
    ...
    public void renderMCTSSuggestion(@NotNull MuZeroConfig config, float @NotNull [] policyTargets) {
        ...
        for (int i = 0; i < boardSize; i++) {
            values[Connect4Action.getRow(config, i)][Connect4Action.getCol(config, i)] = String.format("%2d", Math.round(100.0 * policyTargets[i])) + "%";
        }
    ...
    public void renderNetworkGuess(@NotNull MuZeroConfig config, @NotNull Player toPlay, @Nullable NetworkIO networkOutput, boolean gameOver) {
        ...
        if (networkOutput != null) {
            ...
            if (!gameOver) {
                int boardSize = config.getBoardHeight() * config.getBoardWidth();
                for (int i = 0; i < boardSize; i++) {
                    values[Connect4Action.getRow(config, i)][Connect4Action.getCol(config, i)] = String.format("%2d", Math.round(100.0 * networkOutput.getPolicyValues()[i])) + "%";  // because softmax
                }
                log.debug(EnvironmentBase.render(config, values));
    ...
    public void renderSuggestionFromPriors(@NotNull MuZeroConfig config, @NotNull Node node) {
        ...
        for (int i = 0; i < boardSize; i++) {
            ...
            values[Connect4Action.getRow(config, i)][Connect4Action.getCol(config, i)]
